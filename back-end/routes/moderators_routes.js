...
router.get('/temporary/:rowsCount/:offset', auth, (req, res) => { // admin view all the new moderators
    ...
    pool.getConnection( (err, conn) => {
        ...
        conn.query( "select * from moderators where status = 'temporary' limit ? offset ?",[parseInt(req.params.rowsCount),parseInt(req.params.offset)], (err, results) => {
            conn.release();
            if (err) {
                return res.status(500).end();
            }
